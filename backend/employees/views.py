...
class EmployeeViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['get'])
    def by_exit_quarter(self, request):
        quarter = request.query_params.get('quarter')
        if not quarter:
            return Response({'error': 'Quarter parameter required'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({'error': 'Invalid quarter format. Use format like "2024 Q1"'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if q_num == 1:
            ...
            return Response({'error': 'Invalid quarter number. Must be 1-4'}, status=status.HTTP_400_BAD_REQUEST)
